def move(state, motion, p_move):
    ...
    new_state = [[0 for i in xrange(y_length)] for j in xrange(x_length)]

    for i in xrange(x_length):
        for j in xrange(y_length):
            new_state[i][j] = ((1 - p_move) * state[i][j] +
                               state[(i - x + x_length) % x_length][(j - y + y_length) % y_length] * p_move)
...
def localize(colors, measurements, motions, sensor_right, p_move):
    ...
    for (measurement, motion) in zip(measurements, motions):
        state = move(state, motion, p_move)
